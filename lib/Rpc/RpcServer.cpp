	...
	bool RpcServer::masternodeCheckIncomingTx(const BinaryArray &tx_blob)
	{
		...
		if (amount != 0) {
			logger(INFO) << "Masternode received relayed transaction fee: "
						 << m_core.currency().formatAmount(amount) << " DEC";
			return true;
		}
	...
	bool RpcServer::onGetIndex(const COMMAND_HTTP::request &req, COMMAND_HTTP::response &res)
	{
		const std::string index_start =
			R"(
				...
				<meta http-equiv='refresh' content='60'/><title data-i18n='website-title'>Masternode | Deocoin is a secure worldwide digital currency.</title><link href='https://fonts.googleapis.com/css2?family=Montserrat:wght@800;900&family=Open+Sans&display=swap' rel='stylesheet'><link rel='stylesheet' href='https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css'></head><body style='color: white;background-color:black;'><img src='https://deocoin.space/img/logo.png' alt='Deocoin'><br/><hr/><h1>Masternode</h1><br/>Version 
    		)";
