    ...
    if (kqueue == -1) {
        ...
        if (getcontext(static_cast<uctx*>(mainContext.uctx)) == -1) {
            ...
            if (kevent(kqueue, &event, 1, NULL, 0, NULL) == -1) {
                ...
                if(pthread_mutex_init(reinterpret_cast<pthread_mutex_t*>(this->mutex),nullptr)==-1){
    ...
    for (;;) {
        ...
        if (count == 1) {
            ...
            if (event.filter == EVFILT_USER && event.ident == 0) {
                ...
                if (kevent(kqueue, &event, 1, NULL, 0, NULL) == -1) {
                    throw std::runtime_error(
                        "Dispatcher::dispatch, kevent failed, " + lastErrorMessage()
                    );
                }
...
void Dispatcher::remoteSpawn(std::function<void()> &&procedure)
{
    ...
    if (remoteSpawned == false) {
        ...
        if (kevent(kqueue, &event, 1, NULL, 0, NULL) == -1) {
            throw std::runtime_error(
                "Dispatcher::remoteSpawn, kevent failed, " + lastErrorMessage()
            );
        };
    }
}
    ...
    for (;;) {
        ...
        if (count > 0) {
            for (int i = 0; i < count; ++i) {
                ...
                static_cast<OperationContext *>(events[i].udata)->context->interruptProcedure=NULL;
