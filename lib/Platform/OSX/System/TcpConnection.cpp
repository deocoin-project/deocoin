...
TcpConnection& TcpConnection::operator=(TcpConnection &&other) noexcept(false)
{
    if (dispatcher != nullptr) {
        assert(readContext == nullptr);
        assert(writeContext == nullptr);
        ...
        if (errno != EAGAIN  && errno != EWOULDBLOCK) {
            ...
            if (kevent(dispatcher->getKqueue(), &event, 1, nullptr, 0, nullptr) == -1) {
                ...
                dispatcher->getCurrentContext()->interruptProcedure = [&] {
                    assert(dispatcher != nullptr);
                    assert(readContext != nullptr);
                    auto *context = static_cast<OperationContext *>(readContext);
                    if (!context->interrupted) {
                        ...
                        if (kevent(dispatcher->getKqueue(), &event, 1, nullptr, 0, nullptr) == -1) {
                            throw std::runtime_error(
                                "TcpListener::interruptionProcedure, kevent failed, "
                                + lastErrorMessage()
                            );
                        }
        ...
        if (errno != EAGAIN  && errno != EWOULDBLOCK) {
            ...
            if (kevent(dispatcher->getKqueue(), &event, 1, nullptr, 0, nullptr) == -1) {
                ...
                dispatcher->getCurrentContext()->interruptProcedure = [&] {
                    assert(dispatcher != nullptr);
                    assert(writeContext != nullptr);
                    auto *context = static_cast<OperationContext *>(writeContext);
                    if (!context->interrupted) {
                        struct kevent event;
                        ...
                        if (kevent(dispatcher->getKqueue(), &event, 1, nullptr, 0, nullptr) == -1) {
                            throw std::runtime_error(
                                "TcpListener::stop, kevent failed, " + lastErrorMessage()
                            );
                        }
...
TcpConnection::TcpConnection(Dispatcher &dispatcher, int socket)
    ...
    if (setsockopt(connection, SOL_SOCKET, SO_NOSIGPIPE, (void*)&val, sizeof val) == -1) {
        throw std::runtime_error(
            "TcpConnection::TcpConnection, setsockopt failed, " + lastErrorMessage()
        );
    }
}
