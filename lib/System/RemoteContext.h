...
public:
    ...
    */
    ...
    /*!
        Run other task on dispatcher until future is ready, then return lambda's result,
        or rethrow exception. UB if called more than once.
    */
    T get() const
    {
        wait();
...
private:
    ...
    {
        NotifyOnDestruction(Dispatcher &d, Event &e)
            : dispatcher(d),
              event(e)
        {
        }
